...
public class UserBagCountTest {
	...
	public void testAddCount() throws Exception {
		...
		//2000 yuanbao
		task.setId("10000");
		task.setScript(ScriptHook.TASK_USER_BAG_COUNT.getHook());
...
//		assertEquals(2, list.size());
		...
		BseFinishAchievement partialFinish = (BseFinishAchievement)xinqi.payload;
		assertEquals(50, partialFinish.getPercent());
		user.getBag().addOtherPropDatas(propData);
		Thread.sleep(200);
		System.out.println(list);
		//Total 5 BseModiTask: step from 1 to 5
//		assertEquals(2, list.size());
		...
		assertNotNull("BseFinishAchievement", xinqi);
		partialFinish = (BseFinishAchievement)xinqi.payload;
		assertEquals(100, partialFinish.getPercent());
		user.getBag().addOtherPropDatas(propData);
	}
