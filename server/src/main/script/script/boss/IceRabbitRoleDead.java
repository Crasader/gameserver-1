...
public class IceRabbitRoleDead {
	
	private static final Logger logger = LoggerFactory.getLogger(IceRabbitRoleDead.class);
			...
			for ( int i=0; i<atkBltInfos.size(); i++) {
				...
				if ( isAIUserAttack ) {
					...
					for ( int j=0; j<hurtCount; j++ ) {
						...
						if ( aiUserSession.equals(userInfo.getUserId()) ) {
							...
							break;
						} else if (enemies.contains(userInfo.getUserId()) ) {
				...
				for ( int j=0; j<hurtUserInfos.size(); j++ ) {
					...
					if ( aiUserSession.equals(hurtUserId) ) {
						...
						int bossInitialBlood = boss.getTotalProgress()-boss.getProgress();
						int bossCurrentBlood = hurtUserInfo.getBlood();
						if ( bossCurrentBlood < 0 ) bossCurrentBlood = 0;
						int totalHurt = bossInitialBlood - bossCurrentBlood;

						if ( bossCurrentBlood <= 0 ) {
							logger.info("Boss '{}' is hurted to dead.", roleName);
							HashMap hurtMap = (HashMap)aiUser.getUserData(BossManager.USER_TOTAL_HURT);
							if ( hurtMap == null ) {
								hurtMap = new HashMap();
								aiUser.putUserData(BossManager.USER_TOTAL_HURT, hurtMap);	
							}
							hurtMap.put(roleAttack.getSessionId(), totalHurt);
							isAIDead = true;
							break LOOP;
						} else {
							if ( totalHurt>= boss.getWinProgress() ) {
								//Reach the winning condition
								logger.info("Boss is killed {}", new Object[]{roleName, totalHurt});
								HashMap hurtMap = (HashMap)aiUser.getUserData(BossManager.USER_TOTAL_HURT);
								if ( hurtMap == null ) {
									hurtMap = new HashMap();
									aiUser.putUserData(BossManager.USER_TOTAL_HURT, hurtMap);	
