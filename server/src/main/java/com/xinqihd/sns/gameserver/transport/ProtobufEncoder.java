...
public class ProtobufEncoder extends ProtocolEncoderAdapter {
	...
	private static final Log log = LogFactory.getLog(ProtobufEncoder.class);
	...
	public void encode(IoSession session, Object msg, ProtocolEncoderOutput out) 
		...
		} else if ( msg instanceof XinqiProxyMessage ) {
			XinqiProxyMessage message = (XinqiProxyMessage)msg;
			if (message != null) {
				IoBuffer body = encodeXinqiProxyMessage(message);
				out.write(body);
			}
		}
	}
	...
	public static final IoBuffer encodeXinqiMessage(XinqiMessage message) {
		if ( message.payload != null ) {
			...
			if ( log.isDebugEnabled() ) {
				log.debug("Encoded: ["+message.toString()+"]");
			}
			*/
			return body;
		} else {
			...
			/*
			if ( log.isDebugEnabled() ) {
				log.debug("Encoded empty message["+message.toString()+"]");
			}
			*/
			body.flip();
			return body;
	...
	/**
	 * Encode the XinqiProxyMessage to IoBuffer
	 * 
	 * output.writeInt(_len);    4
	 * output.writeInt(_type);   2
	 * output.writeInt(0);       4
	 ...
	 * 128 Varints</a> integer length field.  For example:
	 * <pre>
	 * BEFORE DECODE (308 bytes)                                  AFTER DECODE (308 bytes)
	 * +------------+---------------------+---------------+       +-----------------------+-------------------+-------------+--------------+
	 * |   Length   | type   |    index   | Protobuf Data |       | Total Length |  Type  | SessionKey Length | SessionKey  | XinqiMessage |
	 * | 0xFFFFFFFF | 0xFFFF | 0xFFFFFFFF |  (300 bytes)  |       |  0xFFFFFFFF  | 0xFFFF |       0xFFFF      |   (X bytes) | (308 bytes)  |
	 * +------------+---------------------+---------------+       +-----------------------+-------------------+-------------+--------------+
	 * 
	 * @param message
	 * @return
	 */
	public static final IoBuffer encodeXinqiProxyMessage(XinqiProxyMessage proxyMessage) {
		if ( proxyMessage.userSessionKey != null ) {
			XinqiMessage message = proxyMessage.xinqi;
			byte[] sessionKeyBytes = proxyMessage.userSessionKey.getRawKey();
			int sessionKeyLength = sessionKeyBytes.length;
			byte[] xinqiBytes = message.toByteArray();
			//Type 2 + Session Key Length 2 + SessionKey + XinqiMessage
			int totalLength = 4 + 2 + 2 + sessionKeyLength + xinqiBytes.length;
			IoBuffer body = IoBuffer.allocate(totalLength);
			body.putInt(totalLength - 4);
			body.putShort((short)0xFFFF);
			body.putShort((short)sessionKeyLength);
