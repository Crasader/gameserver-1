 ...
 *
 */
public class XinqihdCharge {

	private static final Logger logger = LoggerFactory.getLogger(XinqihdCharge.class);
	...
	public static final HttpMessage chargeNotify(HttpMessage httpMessage, String variable) {
		logger.debug("XinqihdCharge request is coming");
		String content = null;
		boolean success = true;
		if ( httpMessage == null ) {
			logger.warn("XinqihdCharge HTTP is null");
			success = false;
		} else {
			...
			if ( content == null || content.length()==0 ) {
				if ( variable != null ) {
					...
					logger.warn("XinqihdCharge HTTP request content is null");
					success = false;
				}
			}
		}
		if ( success ) {
			ScriptResult result = ScriptManager.getInstance().runScript(ScriptHook.CHARGE_XINQIHD, content);
