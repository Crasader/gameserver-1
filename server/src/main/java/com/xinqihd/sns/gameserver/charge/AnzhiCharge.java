...
public class AnzhiCharge {
	...
	private static final Logger logger = LoggerFactory.getLogger(AnzhiCharge.class);
	...
	public static final HttpMessage chargeNotify(HttpMessage httpMessage, String variable) {
		logger.debug("Anzhi Charge request is coming");
		String content = null;
		boolean success = true;
		if ( httpMessage == null ) {
			logger.warn("Anzhi Charge HTTP is null");
			success = false;
		} else {
			...
			if ( content == null || content.length()==0 ) {
				if ( variable != null ) {
					...
					logger.warn("Anzhi Charge HTTP request content is null");
					success = false;
				}
			}
		}
		if ( success ) {
			ScriptResult result = ScriptManager.getInstance().runScript(ScriptHook.CHARGE_ANZHI, content);
