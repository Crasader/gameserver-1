...
public class HuaweiCharge {
	...
	private static final Logger logger = LoggerFactory.getLogger(HuaweiCharge.class);
	...
	public static final HttpMessage chargeNotify(HttpMessage httpMessage, String variable) {
		logger.info("Huawei Charge request is coming");
		String content = null;
		boolean success = true;
		if ( httpMessage == null ) {
			logger.warn("Huawei Charge HTTP is null");
			success = false;
		} else {
			...
			if ( content == null || content.length()==0 ) {
				if ( variable != null ) {
					...
					logger.warn("Huawei Charge HTTP request content is null");
					success = false;
				}
			}
		}
		if ( success ) {
			ScriptResult result = ScriptManager.getInstance().runScript(ScriptHook.CHARGE_HUAWEI, content);
