 ...
 *
 */
public class TaskEquipmentPanel extends AbstractTaskPanel {
	
	private static TaskEquipmentPanel instance = new TaskEquipmentPanel(); 

	public TaskEquipmentPanel() {
		init();
	}
	
	public static TaskEquipmentPanel getInstance() {
		return instance;
	}

	public void init() {		
		//创建工作区域
		String collection = "equipments_new";
		super.initModel(collection);
		
		hiddenFields.add("class");
		hiddenFields.add("index");
		hiddenFields.add("sName");
