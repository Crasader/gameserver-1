...
public class AccountSearchService extends SwingWorker<Void, Void> {
	...
	protected Void doInBackground() throws Exception {
		...
		if ( !preciMatch ) {
			...
			DBObject query = MongoUtil.createDBObject("serverRoles.roleNames", regex);
			...
			DBObject query = MongoUtil.createDBObject("serverRoles.roleNames", userNameOrId);
			list = MongoUtil.queryAllFromMongo(query, databaseName, namespace, collection, null);
			if (list == null || list.size() == 0 ) {
				UserId userId = UserId.fromString(userNameOrId);
				query = MongoUtil.createDBObject("_id", userId);
