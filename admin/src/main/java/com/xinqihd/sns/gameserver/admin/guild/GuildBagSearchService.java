...
public class GuildBagSearchService extends SwingWorker<Void, Void> {
	...
	protected Void doInBackground() throws Exception {
		...
		if ( !preciMatch ) {
			...
			DBObject query = MongoUtil.createDBObject("_id", regex);
			list = MongoUtil.queryAllFromMongo(query, databaseName, namespace, collection, null);
		} else {
			DBObject query = MongoUtil.createDBObject("_id", userNameOrId);
			list = MongoUtil.queryAllFromMongo(query, databaseName, namespace, collection, null);
		}
