...
public class MongoUtil {
	...
	private static final String[] USER_COLL = {
		"accounts", "users", "bags", "relations", "unlocks, uuids",
	};
	private static final String[] CFG_COLL = {
		...
		"servers", "rewards", "tips", "promotions", "cdkeys"
	};
	...
	static {
		...
		try {
			...
			defaultMongo = initMongo(mongoHost, mongoPort);
		} catch (Exception e) {
	...
	public static final Object constructObject(DBObject dbObject) {
		...
		try {
			...
			if ( className.startsWith(".") ) {
				className = MongoDBUtil.CLASS_PREFIX.concat(className);
			}
...
//					}
		...
		for ( String key : keys ) {
			...
			if ( keyClassObj != null ) {
				...
				if ( keyClassName.startsWith(".") ) {
					keyClassName = MongoDBUtil.CLASS_PREFIX.concat(keyClassName);
				}
				Class keyClass = MongoUtil.class.getClassLoader().loadClass(keyClassName);
				if ( keyClass.isEnum() ) {
	...
	public static final DB getDB(String databaseName,
		...
		if ( mongo == null ) {
			if ( namespace == null ) {
				key = StringUtil.concat(databaseName, DOT, collection);
			} else {
				key = StringUtil.concat(databaseName, DOT, namespace, DOT, collection);
			}
			mongo = mongoMap.get(key);
			if ( mongo == null ) {
				mongo = defaultMongo;
				ArrayList<Mongo> mongoList = new ArrayList(mongoMap.values());
				for ( Mongo m : mongoList) {
					refreshMongoDatabase(m);
				}
				DB db = mongo.getDB(databaseName);
				logger.debug("Failed to find Mongo by key:{}. Need refresh", key);
				return db;
			} else {
				DB db = mongo.getDB(databaseName);
				return db;
